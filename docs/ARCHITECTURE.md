...
> | Conflict resolution engine | **done** (engine + REST) | Deterministic decision table at the `add_node` MERGE seam (`backend/conflict.py`). Four lanes by `FactConfidence` rung: AUTO_MERGE (equal-after-normalize), AUTO_PICK (HUMAN-overrides or higher-rung-wins), LLM_TRIAGE (both INFERRED — lane stub, live Gemini call deferred), ESCALATE (tied at confident rung). REST surface: `GET /api/conflicts` + `POST /api/conflicts/{id}/resolve`. Resolutions go through `edit_node` so they carry `FactConfidence.HUMAN` provenance and are reversible. UI inbox not yet. |
...
> | **Flow 4** | Human Resolves Conflict (conflict queue) | **partial** (engine + REST) | Conflict detection at MERGE; queue + resolve via `/api/conflicts*` endpoints; resolutions write `FactConfidence.HUMAN` provenance through `edit_node` so they're reversible | UI inbox; live LLM triage lane (currently lane stub) |
